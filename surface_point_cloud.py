...
class SurfacePointCloud:
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal',
                                normal_sample_count=11, min_size=0, return_gradients=False, rate=0.94):
        query_points = []
        surface_sample_count = int(number_of_points * rate) // 2
    ...
    def sample_sdf_uni(self, number_of_points=500000, sign_method='normal',
                       normal_sample_count=11, return_gradients=True):
        unit_sphere_points = sample_uniform_points_in_unit_sphere(number_of_points)
        query_points = np.array(unit_sphere_points).astype(np.float32)
    ...
    def sample_on_surface(self, number_of_points=500000, use_scans=True, sign_method='normal',
                          normal_sample_count=11, return_gradients=True):
        surface_points = self.get_random_surface_points(number_of_points, use_scans=use_scans)
        query_points = np.array(surface_points).astype(np.float32)
...
def get_surface_point_cloud(mesh, surface_point_method='scan', bounding_radius=None, scan_count=100,
    ...
    if surface_point_method == 'scan':
        ...
        return sample_from_mesh(mesh, sample_point_count=sample_point_count,
                                calculate_normals=calculate_normals)
    else:
...
def grad_correction(sdf, grad, pos_direction=None):
    if pos_direction is None:
        pos_direction = [0, 0, 1]
    udf = np.where(sdf < 0, -sdf, sdf)
    g = deepcopy(grad)
    g[sdf < 0] *= -1
    g = np.dot(g, np.array(pos_direction))
    udf[g < 0] *= -1
    return udf, grad
...
def sample_sdf_near_surface(mesh, number_of_points=500000, surface_point_method='scan', sign_method='normal',
                            ...
                            min_size=0, return_gradients=True, rate=0.94):
    ...
    return surface_pcl.sample_sdf_near_surface(number_of_points, surface_point_method == 'scan', sign_method,
                                               normal_sample_count, min_size, return_gradients, rate)


def sample_on_surface(mesh, number_of_points=500000, surface_point_method='scan', sign_method='normal',
                      scan_count=100, scan_resolution=400, sample_point_count=10000000, normal_sample_count=11,
                      min_size=0, return_gradients=True, rate=0.94):
    if surface_point_method == 'sample' and sign_method == 'depth':
        print("Incompatible methods for sampling points and determining sign, using sign_method='normal' instead.")
    ...
    freespace_points = surface_pcl.sample_sdf_uni(freespace_points_num, sign_method,
                                                  normal_sample_count, return_gradients)
    return surface_points, freespace_points
