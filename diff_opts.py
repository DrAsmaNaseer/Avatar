...
def gradients(y, x, grad_outputs=None):
    if grad_outputs is None:
        grad_outputs = torch.ones_like(y)
    return torch.autograd.grad(y, x, grad_outputs=grad_outputs, create_graph=True)


def gradient_3d_by_3d(y, x):
    ...
    return torch.stack([grad_u, grad_v, grad_w], dim=2)  # gradient of deformation wrt. input position
...
def symmetric_dirichlet_energy(y, x):
    jac, status = jacobian(y, x, True)
    jac_inv = torch.linalg.inv(jac)  # torch.linalg.pinv()
    return (torch.sum(jac.flatten(start_dim=-2) ** 2, dim=-1) + torch.sum(jac_inv.flatten(start_dim=-2) ** 2,
                                                                          dim=-1)) / 2
