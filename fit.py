...
def get_dataset(config):
    ...
    fit_ids = []
    with open(config.fit_list, 'r') as f:
        for line in f:
            line = line.strip()
            if not len(line) == 0:
                fit_ids.append(line)
    fit_ids.sort(key=sort_key)

    fit_data = NormalDataset(config.data_path, fit_ids, config.exp_types, config.sample_num,
    ...
    print('Fitting {} samples with {} identities.\n'.format(len(fit_data), len(fit_ids)))
    return fit_dataloader, fit_data, len(fit_ids)
...
def main(args):
    ...
    fit_dataloader, fit_data, fit_id_num = get_dataset(config)
    ...
    save_path = os.path.join(reload_path, 'fit')
    ...
    id_embedding = torch.nn.Embedding(fit_id_num * len(config.exp_types), config.network_params.id_embedding_dim).to(
        device)
    torch.nn.init.normal_(id_embedding.weight, mean=0, std=0.01)  # std=1/math.sqrt(self.id_dim) or 0.01
    exp_embedding = torch.nn.Embedding(fit_id_num * len(config.exp_types), config.network_params.exp_embedding_dim).to(
        device)
    ...
    with tqdm(total=config.epoch) as tqdm_bar:
        for epoch in range(1, config.epoch + 1):
            ...
            for data_dict, _ in fit_dataloader:
                ...
                idx = data_dict['exp'].long() * fit_id_num + data_dict['id'].long()
        ...
        cd, f, errors = summary.fit_decoder(config, decoder.module, fit_data, exp_embedding, id_embedding,
                                            os.path.join(work_dir, 'fit_results'), device, fit_id_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", help="config path.", default="fit_lif_facescape_sub.yaml")
